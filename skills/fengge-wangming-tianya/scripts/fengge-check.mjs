...
const alLine = (al.out.split('\n').find(l => l.includes('带内比例')) || '').trim();
console.log(alLine || al.out.slice(0, 300));
const score = Number((alLine.match(/=\s*([\d.]+)%/) || [])[1] || 0) / 100;
if (score < MIN_ALIGN) { console.log(`✗ 带内比例 ${(score * 100).toFixed(0)}% 低于阈值 ${(MIN_ALIGN * 100).toFixed(0)}%`); bad++; }

console.log('\n--- 结论 ---');
console.log(`历史文案 FAIL 率 ${(failedShare * 100).toFixed(0)}%（这批本来就该 FAIL：免责句/对仗是旧规则产物）`);
console.log(`黄金批次统计带内比例 ${(score * 100).toFixed(0)}%（阈值 ${(MIN_ALIGN * 100).toFixed(0)}%）`);
