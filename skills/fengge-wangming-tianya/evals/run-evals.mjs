...
// 被 fengge-check.mjs import 时只导出 judge / runSelftest，不执行 CLI
...
/** 跑内置正反例，验证 runner 本身。返回 { rows, total, mismatched, saved }。 */
export function runSelftest({ verbose = true } = {}) {
  let bad = 0;
  const rows = [];
  ...
  const saved = save({ at: new Date().toISOString(), kind: 'selftest', total: SELFTEST.length, mismatched: bad, rows });
  return { rows, total: SELFTEST.length, mismatched: bad, saved };
}
...
if (invokedDirectly) {
  ...
  if (argv.includes('--selftest')) {
    const r = runSelftest();
    console.log(`\nselftest：${r.total - r.mismatched}/${r.total} 符合预期 → ${r.saved}`);
    process.exit(r.mismatched ? 1 : 0);
  }
